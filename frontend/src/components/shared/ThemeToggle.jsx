import { useTheme } from '../../contexts/ThemeContext';

const ThemeToggle = ({ className = '', size = 'md' }) => {
    const { isDark, toggleTheme, darkModeEnabled } = useTheme();

    if (!darkModeEnabled) return null;

    const sizes = {
        sm: 'w-8 h-8',
        md: 'w-9 h-9',
        lg: 'w-10 h-10',
    };

    const iconSizes = {
        sm: 'w-4 h-4',
        md: 'w-[18px] h-[18px]',
        lg: 'w-5 h-5',
    };

    return (
        <button
            onClick={toggleTheme}
            className={`${sizes[size]} inline-flex items-center justify-center rounded-xl transition-all duration-300 ${isDark
                ? 'bg-gray-800 hover:bg-gray-700 text-amber-400 border border-gray-700 hover:border-gray-600'
                : 'bg-gray-100 hover:bg-gray-200 text-gray-600 border border-gray-200 hover:border-gray-300'
                } ${className}`}
            title={isDark ? 'Switch to light mode' : 'Switch to dark mode'}
            aria-label={isDark ? 'Switch to light mode' : 'Switch to dark mode'}
        >
            {isDark ? (
                /* Sun icon */
                <svg className={`${iconSizes[size]} theme-toggle-icon`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
                </svg>
            ) : (
                /* Moon icon */
                <svg className={`${iconSizes[size]} theme-toggle-icon`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
                </svg>
            )}
        </button>
    );
};

export default ThemeToggle;
